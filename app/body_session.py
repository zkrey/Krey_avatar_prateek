...
def analyze_body(
    ...
    Height is the scale anchor. Non-measurable frames (headshots, half-body, group) are
    gated out with a reason, never measured into the result.
    """
    import os
    import cv2
    ...
    for path in sorted(image_paths):
        ...
        frame = body.run_pose_landmarker(img, model_path)
        gate = core.body_measurable(frame.visibility) if frame.detected else \
            {"measurable": False, "reason": "no_person_detected", "coverage": 0.0}
