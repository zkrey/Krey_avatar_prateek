...
| `feat/service-a-backend` | Service A: backend + docs + `/tester` console. |
| `feat/frontend-tester-console` | Full Service-A QA console in `app/webtest.html`. |

`claude/<name>-<id>` branches are **auto-generated by automated sessions** — `claude/`
marks the session, `<name>` says what's inside, the id is just a uniqueness suffix.
One source per line of work — no duplicate/alias branches carrying the same commits.

**Naming going forward (non-automated branches):** `type/scope-detail` — e.g.
`feat/frontend-tester`, `fix/fit-score`, `docs/doctrine`. The name must say *what's
inside* before you open it. One concern per branch; branch off the branch that already
holds the code you build on, not always `main`. See `CONTRIBUTING.md`.
