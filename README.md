...
| `feat/service-a-backend` | Service A: backend + docs + `/tester` console (aka `claude/krey-avatar-setup-*`). |
| `feat/frontend-tester-console` | Full Service-A QA console in `app/webtest.html` (aka `claude/krey-frontend-*`). |

`claude/<name>-<id>` branches are **auto-generated by automated sessions** — `claude/`
marks the session, the id is just a uniqueness suffix. Each has a readable
`feat/…` alias at the same commits (the name humans should use).
...
One concern per branch; branch off the branch that already holds the code you build
on, not always `main`. See `CONTRIBUTING.md`.
