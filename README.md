...
| `docs/` | `backend_status.md`, `DOCTRINE.md`, `scope_bakeins.md`, `DEPLOY.md`, `FEEDBACK_LOOP.md`. |
...
**`main` is the single source of truth** — the complete app (backend + render/bench
work + the full `/tester` QA console). Deploy from `main`; branch from `main`.

| Branch | Holds |
|---|---|
| `main` | ✅ Canonical. Everything, merged. Deploy this. |

All earlier working branches (`feat/*`, `claude/*`) were consolidated into `main` and
retired — **do not** revive them or push parallel copies. Start new work from `main`.

**Naming for new branches:** `type/scope-detail`, lowercase — e.g. `feat/frontend-tester`,
`fix/fit-score`, `docs/doctrine`. The name must say *what's inside* before you open it;
one concern per branch; open a PR back into `main`. See `docs/BRANCHING_SOP.md` and
`CONTRIBUTING.md`.
